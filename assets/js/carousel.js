...
export class Carousel {
	...
	setup(elm) {
		const container = elm.getElementsByClassName('js-carouselContainer')[0];
		const rell = elm.getElementsByClassName('js-carouselReel')[0];
		this.carouselData = {
			root: elm,
			container: container,
			reel: rell,
	...
	createPagination() {
		this.carouselData.root.insertAdjacentHTML('beforeend', '<nav class="pagination"></nav>');
		this.carouselData.pagination = this.carouselData.root.getElementsByClassName('pagination')[0];
	}
	createControls() {
